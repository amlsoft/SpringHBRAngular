...
app.config(['$httpProvider', function($httpProvider) {
        ...
        $httpProvider.defaults.headers.common['Authorization'] = 'Basic d2VudHdvcnRobWFuOkNoYW5nZV9tZQ';
}]).config(["$routeProvider", function($routeProvider) {
	// Global routes
	$routeProvider.when("/version", { templateUrl: "version-content.html" });
}]);

//app.config('restangular', function(Restangular) { /// YAK!
//    Restangular.setBaseUrl('http://localhost:8084/alchemytec');
//    Restangular.setDefaultRequestParams({token: 'token value'})
//    Restangular.setDefaultHeaders({ authentication: 'bearer ' + '12564879' });
//});
